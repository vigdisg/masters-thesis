...
class RBF(nn.Module):
    ...
    def forward(self, X):
        ...
        #dim 0 is the bandwidth dimension
...
y = torch.tensor(y, dtype=torch.float32).reshape(-1, 1) #each row is a different data point

# DEFINE MODEL:

N_input = 100 #number of uniformly sampled input values
...
X = torch.tensor(X, dtype=torch.float32).reshape(500, N_input)
